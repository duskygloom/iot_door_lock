...
void printLine(const String &text, int line, LineAlignment align = LINE_LEFT, bool eraseLine = true)
{
    /**
     * @note
     * Lines displayed using this function will get erased
     * after a set timeout.
     */
...
void enrollFingerprint()
{
    ...
    if (getFingerTemplate(1) == FINGER_OK && searchFinger() == FINGER_OK) {
        printLine("Template exists.", 2, LINE_CENTER);
        return;
    }
    // create finger model
    switch (createFingerModel()) {
        case FINGER_OK:
            break;
        case FINGER_NOT_MATCHED:
            printLine("Mismatch, retry...", 2, LINE_CENTER);
            return;
        default:
            printLine("Retry...", 2, LINE_CENTER);
            return;
    ...
    switch (storeFingerModel()) {
        case FINGER_OK:
            ...
            printLine(message, 2, LINE_CENTER);
            return;
        default:
            printLine("Retry...", 2, LINE_CENTER);
            return;
...
void handleIR()
{
    ...
    if (IrReceiver.decode()) {
        ...
        if (command == KEY_OK) relayOn();
        printLine(command == KEY_OK ? "Success!" : "Failure!", 2, LINE_CENTER);
...
void controlFinger()
{
    ...
    // not matched
        ...
        printLine("Not matched!", 3, LINE_CENTER);
    // user matched
    else if (fingerID >= 1 && fingerID <= 127) {
        printLine("Matched!", 3, LINE_CENTER);
        relayOn();
    }
}
...
#define ENROLL_ON (digitalRead(ENROLL_BUTTON_PIN) == LOW)

void controlEnrollment()
{
    lcd.printLine("Press admin...", 2, LINE_CENTER);
    unsigned long start = millis();
    uint16_t fingerID = getFingerprintID();
    while (millis() < start + ENROLL_TIMEOUT) {
        if (fingerID < 1 || fingerID > MAX_ADMINS) {
            delay(SCAN_FAST_TIMEOUT);
    ...
    printLine("Failure.", 2, LINE_CENTER);
}
...
void loop()
{
    if (ENROLL_ON) controlEnrollment();
